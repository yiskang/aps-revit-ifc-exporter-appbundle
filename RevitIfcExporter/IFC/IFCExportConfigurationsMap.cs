...
namespace BIM.IFC.Export
{
    ...
    public partial class IFCExportConfigurationsMap
    {
...
#if SinceRVT2025
        private LinkedFileExportAs ParseLinkedFilesAs(string val, Entity configEntity)
        {
        ...
        public void AddSavedConfigurations(Document document)
        {
            try
            {
                ...
                if (m_OldSchema == null)
                {
                    ...
                    if (m_OldSchema != null)
                    {
                        foreach (DataStorage storedSetup in GetSavedConfigurations(m_OldSchema, document))
                        {
                            ...
                            {
                                var userDefinedPsetsFileName = Path.GetFileName(configEntity.Get<string>(s_setupExportUserDefinedPsetsFileName));
                            ...
                            {
                                var userDefinedParameterMappingFileName = Path.GetFileName(configEntity.Get<string>(s_setupExportUserDefinedParameterMappingFileName));
                            ...
                            if (fieldExportLinkedFiles != null)
                                configuration.ExportLinkedFiles = ParseLinkedFilesAs(s_setupExportLinkedFiles, configEntity);
#else
                            Field fieldExportLinkedFiles = m_OldSchema.GetField(s_setupExportLinkedFiles);
                            if (fieldExportLinkedFiles != null)
                ...
                if (m_mapSchema == null)
                {
                    ...
                    if (m_mapSchema != null)
                    {
                        foreach (DataStorage storedSetup in GetSavedConfigurations(m_mapSchema, document))
                        {
                            ...
                            {
                                var userDefinedPsetsFileName = Path.GetFileName(configMap[s_setupExportUserDefinedPsetsFileName]);
                                configuration.ExportUserDefinedPsetsFileName = Path.Combine(Directory.GetCurrentDirectory(), userDefinedPsetsFileName);
                            }
                            ...
                            {
                                var userDefinedParameterMappingFileName = Path.GetFileName(configMap[s_setupExportUserDefinedParameterMappingFileName]);
                                configuration.ExportUserDefinedParameterMappingFileName = Path.Combine(Directory.GetCurrentDirectory(), userDefinedParameterMappingFileName);
                            }
                ...
                if (m_jsonSchema == null)
                {
                    ...
                    if (m_jsonSchema != null)
                    {
                        foreach (DataStorage storedSetup in GetSavedConfigurations(m_jsonSchema, document))
                        {
                            try
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(configuration.ExportUserDefinedPsetsFileName))
                                {
        ...
        /// </summary>
        public void UpdateSavedConfigurations(IFCExportConfigurationsMap initialConfigs, Document document)
        {
            ...
            if (m_mapSchema != null)
            {
                ...
                if (oldSavedConfigurations.Count > 0)
                {
                    Transaction deleteTransaction = new Transaction(document,
        ...
        private IList<DataStorage> GetSavedConfigurations(Schema schema, Document document)
        {
